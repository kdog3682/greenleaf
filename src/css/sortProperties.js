...
const cssKeysPartitionOrdering = {
    // Positioning
    positioning: [
        "position",
        "z-index",
...
function sortProperties(properties) {
    const keysets = Object.values(cssKeysPartitionOrdering)
    const storage = new Storage()

    properties.forEach((item, i) => {
        const [k, v] = item
        const finder = (keyset) => {
            return keyset.includes(k)
        }
        const index = keysets.findIndex(finder)
        assert3(index != -1, {
            message: "the index is -1",
            args: item,
        })

        storage.add(index, item)
    })
    return storage.values.flat()
}
