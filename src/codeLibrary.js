...
function codeLibrary({ node, getText }) {
    const ref = pythonRef
    function callback(node) {
        ...
        if (ref.hasOwnProperty(node.name)) {
            Object.assign(data, ref[node.name](node))
        }
        if (Object.values(data).some(isNull)) {
            throw console.log(console.getText(node))
        }
        return data
    }
    const data = iterateTopLevel(node, callback)
    return walk4(data, (x) => isString(x) ? x : getText(x))
}
...
function ltf(x) {
    ...
    if (isString(x)) {
        if (x.includes('|')) {
            const candidates = split(x, /\|/)
            return function lambda(node) {
                return inside(candidates, node.name)
            }
        }
        return function lambda(node) {
            return node.name == x
    ...
    return function lambda(node) {
        return x.test(node.name)
    }
}
function findChildNode(node, query) {
    let fn = ltf(query)
...
function childNodeSelector(node, ...queries) {
    return _childNodeSelector(node, queries, null)
}

function childNodeSelectorAll(node, ...queries) {
    return _childNodeSelector(node, queries, Array) || []
}
function _childNodeSelector(node, queries, mode) {
    for (let i = 0; i < queries.length; i++) {
        const query = queries[i]
        const match = findChildNode(node, query)
        if (!match) {
            return
        }
        node = match
...
function getNodeIdentifier(node) {
    return {
        identifier: findChildNode(node, "VariableName | MemberExpression")
    }
}
const pythonRef = {
    VariableDeclaration: getNodeIdentifier,
    AssignStatement: getNodeIdentifier,
    ClassDefinition: getNodeIdentifier,
    FunctionDefinition(node) {
        const params = childNodeSelectorAll(node, 'ParamList', 'VariableName')
        return {
...
// const {node, getText} = createState(s)
// traverseBlue(node, getText)
// const state = await createState(s)
// console.log(codeLibrary(state))
// traverseBlue(state)
//
...
// const files = [
    // "/home/kdog3682/PYTHON/base.py",
    // "/home/kdog3682/PYTHON/run.py",
    // "/home/kdog3682/PYTHON/next.py",
    // "/home/kdog3682/PYTHON/utils.py",
    // "/home/kdog3682/PYTHON/vim_package_manager.py",
    // "/home/kdog3682/PYTHON/chatgpt_vim_python_executor.py",
// ]
// const s2 = join(files.map(read))

// const state = await createState(s)
// const state = await createState(join(files.map(read)))
// const lib = codeLibrary(state)
