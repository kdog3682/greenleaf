...
function viewTree(x, text) { 
    const data = walkTree(x, getStatef(text))
    console.log(data)
    return data
}
function walkTree(x, fn, topLevel) {
...
class Environment {
    ...
    declareVar(name, value, constant) {
        if (constant && this.variables.has(name)) {
            panic('cannot redeclare constant $1', name)
        }
        this.variables.set(name, value)
        return value
    }
    createEnv(items) {
        const child = new Environment(this, items, this.options)
...
function traverseBare(state) {

    function onLeaf({name}, value) {
        return { name, value }
    }
    function onBranch({name}, children) {
...
function simpleTraverse(state, {onLeaf, onBranch}, ...args) {
    ...
    function run(node) {
        ...
        if (child) {
            const children = []
            do {
                push2(children, run(child))
            } while ((child = child.nextSibling))

            return onBranch(node, children, ...args)
        } else {
            const text = state.getText(node)
            return onLeaf(node, text, ...args)
        }
    }
}
