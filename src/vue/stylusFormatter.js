...
// const base = recursivelyDeletePropertyFields(copy(ast), fields)
// clipper(base)
// console.log(base)
// console.loggg(scaffolder(base, {regex: /^__(\w+)/, useMatchKey: false}))


const stylusVisitors = {
    Root(ast) {
        return join(ast.nodes.map(this.visit))
    },
    Group(ast) {
        function groupWrapper(nameParts, bodyParts) {
            const name = nameParts.join('')
            const body = bodyParts.join('\n')
            return name + newlineIndent(body)
        }
        const nameParts = ast.nodes.map(this.visit)
        const bodyParts = this.visit(ast.block)
        return groupWrapper(nameParts, bodyParts)
    },
    Selector(ast) {
        return ast.segments.map(this.visit).join('')
    },
    Block(ast) {
        const properties = ast.nodes.map(this.visit)
        const sortedProperties = sortProperties(properties)
        return strReduce(sortedProperties, '$1: $2')
    },
    Property(ast) {
        const key = ast.segments.map(this.visit).join('')
        const value = this.visit(ast.expr)
        return [key, value]
    },
    Expression(ast) {
        return ast.nodes.map(this.visit).join(' ')
    },
    Ident(ast) {
        if (ast.val.__type == 'Null') {
            return ast.name
        }
        todo()
    },
...
const stylusVisitorConfig = {
    visitors: stylusVisitors,
    entry: '__type',
    transform: null,
}
...
const prepare = (s) => copy((new Parser(s)).parse())
const stylusFormatter = visitf(prepare, stylusVisitorConfig)
// console.log(stylusFormatter(stylusCode))
