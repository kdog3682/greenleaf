...
function buildTag(tag, attrs, children) {
    ...
    const isTextChild = children.length == 1 &&
    ...
    if (exists(block)) {
        ...
        const computedBlock = alignNestedArray(
            sort(block, sorter),
        ).map(callback)
        if (
...
// const s = `p.asdf.smartDedent5(asdf = 'AAA' asaaa = 1) bye`
// console.log(pugFormatter(s))
