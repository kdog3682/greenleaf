...
const master = [
    ...
    },
    {
        ...
        matchers: [
            {
                attributeKey: "lang",
                attributeValue: "stylus",
                formatter: stylusFormatter,
                prepare: basicPrepare,
            },
        ],
    },
...
function vueFormatter(s) {
    ...
    const callback = (node) => {
        ...
        const wrapper = (s) => {
            ...
            const payload = {
                tag: reference.tag,
                attrs: {
                    [m.attributeKey]: m.attributeValue,
                },
                text: "\n\n" + s.trim() + "\n\n",
                options: {
                    indentText: false,
    ...
    const results = sort(values, (x) => x[0]).map((x) => x[1])
    return join(results)
}
...
function getAttributes(node, getText) {
    ...
    return reduce(attrNodes, (node) => {
        const key = getText(node.getChild("AttributeName"))
        const valueNode = node.getChild("AttributeValue")
        const value = valueNode
            ? removeQuotes(getText(valueNode))
            : ""
        return [key, value]
    })
}
