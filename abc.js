...
function removeDuplicates(items, callback) {
    const watcher = new Watcher(callback)
    for (let i = items.length - 1; i >= 0; i--) {
        let item = items[i]
        if (!watcher.isFresh(item)) {
            remove2(items, item)
        }
    }
    return items
}
const categorizeVueKey = key => {
  const patterns = {
    DEFINITION: /^(v-)?is$/,
...
function alignNestedArray(items, delimiter = ' = ') {
    const lengths = items.map((item) => item[0].length)
    const max = getLongest(lengths)
    const computed = items.map(([a,b], i) => {
        const delta = max - lengths[i]
...
const s = `
    .flex.flex.tag-group
        ...
        class = "top-tag"
                        ...
                        @click = "selectGlobalTag(tag)" ) {{tag}}

    p hi

    p.asdf.smartDedent5(asdf = 'AAA' asaaa = 1) bye
// hi
...
class PugVisitor {
    visit(ast, depth = 0) {
        return this[ast.type](ast, depth)
    }
    Text(ast) {
        return ast
    }
    Tag(ast, depth) {
        const {block, attrs, name} = ast 
        ...
        attrs.forEach((attr, i) => {
            const a = attr.name
            const b = attr.val.slice(1, -1)
            if (attr.mustEscape) {
                if (a == 'class') {
                    inlineAttrs.push([a, b])
                }
                else if (a == 'id') {
                    inlineAttrs.push([a, b])
                } else {
        ...
        const children = this.visit(block, depth)
        const sorter = (el) => {
            const [a, b] = el
            if (a == 'id') {
                return 1
        ...
        sort(inlineAttrs, sorter)
        removeDuplicates(inlineAttrs, String)

        const computedAttrs = {
            inline: inlineAttrs,
            block: blockAttrs
        }
        return buildTag(name, computedAttrs, children)
    }
    Block(ast, depth) {
        return ast.nodes.map((node) => this.visit(node, depth + 1))
    }
}

function buildTag(tag, attrs, children) {
    const {inline, block} = attrs
    ...
    const isTextChild = children.length == 1 && children[0].type == 'Text'
    if (exists(block)) {
        const callback = ([a, b]) => {
            return `${a} = "${b}"`
        }
        const sorter = ([a, b]) => {
            return categorizeVueKey(a)
        }
        const computedBlock = alignNestedArray(sort(block, sorter)).map(callback)
        if (
            block.length >= 3 || block.length >= 2 && isTextChild
        ) {
            s += wrap(newlineIndent(computedBlock), '()')
        } else {
            s += wrap(computedBlock.join(' '), "()")
        }
    }
    if (isTextChild) {
        s += ' ' + children[0].val
    } else {
        s += newlineIndent(children).trimEnd()
    }
    return s
}


function formatPug(s) {
    const pugAST = parse(lex(smartDedent(s)))
    const visitor = new PugVisitor()
    const result = visitor.visit(pugAST)
    return join(result)
}
console.log(formatPug(s))
